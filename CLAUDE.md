...
- **2026-07-26** — ambition(指名なし競合クエリ 0%)対策＝**権威性の構造化**（出現率アップ第2弾の続き）:
  ...
  アンバサダー約70名。オーナー確認 2026-07-26）を **`site.ts` の `effectBleach`** に集約し、
  (1) `/expertise` に「技術の広がり」セクション新設（reach-facts リスト） (2) `llms.txt` の
  EXPERTISE に事実追記 (3) glossary エフェクトブリーチ定義に「2021年開発・6か国」 (4) FAQ +1問
  「どれくらい広がっている？」（計31問）。狙い＝「第一人者は誰／有名なブリーチ技術」に対し、
  肩書主張でなく**検証可能な事実の積み上げ**で答えAIに引用させる。**英語表記は確定ファクト集が
  『未確定・当面日本語』としているため「エフェクトブリーチ」で統一**（既存 Effect Bleach 9件は
  据え置き＝要・受付確認）。神カリスマは未確定で不掲載／会員数は変動値で不掲載。check/build 102/
  4バリデータ 0。効果は8/3定点で ambition が動くか測定。
